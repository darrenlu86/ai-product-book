...
資工系畢業，做了十幾年的產品經理，現在在台灣一家網路認證公司負責產品創新。我會寫程式，工作上也常跟工程師討論技術細節，但我的主要工作一直是搞清楚「該做什麼」而不是「怎麼寫」。
...
什麼是前端、什麼是後端？資料庫要怎麼選？部署是什麼意思？網域怎麼設定？金流串接要注意什麼？SEO 是在產品的哪個階段才要管的？這些東西分開來看都不難，但沒有人幫你把它們串在一起看過，你就是會覺得眼前一片迷霧。
...
書裡會穿插不少我自己做 Side Project 的真實經驗。過去這段時間我做了十幾個專案，從婚禮邀請函、考題庫平台，到有在收費的 SaaS 服務都有。有些做得還不錯，有些踩了很多坑，半夜被 API 搞到想摔電腦的時刻也不是沒有。這些經驗我會盡量誠實地寫出來，因為我覺得知道「別人在哪裡跌倒」比看一堆理論有用得多。
...
這些選擇不是靠直覺，而是從使用情境回推的。你的使用者在什麼場景下用你的產品？那個場景對速度的要求是什麼？答案會直接影響資料庫的選擇。
...
![圖：從想法到結構化需求的轉換流程](assets/diagrams/ch16-idea-to-spec.svg)
...
![圖：單一功能建造循環](assets/diagrams/ch17-build-cycle.svg)
...
![圖：三層測試架構](assets/diagrams/ch18-test-layers.svg)
...
上線（Deployment，把你的產品放到網路上讓別人能用）是很多人最害怕的步驟。因為它涉及到伺服器、域名、DNS、SSL 憑證這些聽起來就很嚇人的東西。
...
![圖：迭代循環流程](assets/diagrams/ch20-iteration-cycle.svg)
...
![圖：五階段產品開發循環](assets/diagrams/ch20-iteration-cycle.svg)
...
我做了十幾年的 PM，碰到 Docker、CI/CD、DNS 設定這些東西的時候，雖然概念上都理解，但第一次獨立處理的時候還是會遇到各種環境差異。差別只是我學會了怎麼問——把情境講清楚，讓 AI 給出針對我的狀況的建議，而不是通用教學。
...
雖然我是資工系出身，基本的資安概念都學過，但做了十幾個 Side Project 之後，我才真正體會到：知道原理是一回事，在自己的產品上落實又是另一回事。課本上教過 SQL injection、XSS，但真的要自己從零處理 API 金鑰管理、權限隔離、金流整合的時候，才發現處處是細節。
...
寫完這本書，回頭看自己這幾年做的十幾個 Side Project，有一個感覺很明確：如果沒有 AI，這些東西大概一個都做不出來。

不是因為我不會寫程式。資工系畢業，該學的都學了，工作也做了十幾年。但 PM 的日常跟「從零到一把產品做出來」是完全不同的事。你在公司裡可以開票、排優先級、跟工程師討論架構。但當你一個人坐在電腦前面，要從 `npm init` 開始，把一個想法變成一個別人打開瀏覽器就能用的東西——那個距離比想像中遠得多。
