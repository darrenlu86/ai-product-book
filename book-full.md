---
title: "AI 時代的產品開發思維"
subtitle: "不用會寫程式，也能用 AI 打造完整軟體產品的方法論"
author: "呂紹民（Darren Lu）"
date: "2026"
lang: zh-TW
toc: true
toc-depth: 2
...
我第一次真正理解「軟體產品」這件事，是在做謝卡醬（weddingcard-saas）的時候。

那是一個婚禮感謝卡的線上服務——新人上傳賓客名單，系統自動生成個人化的感謝小卡，可以用 Email 寄出去。聽起來很簡單對吧？我當初也這麼覺得。

然後我開始列需要做的事：要有一個網頁讓新人操作、要有地方存賓客資料、要能生成卡片圖片、要串金流讓人付費、要能寄 Email、要部署到網路上讓所有人都能用。列完之後我盯著螢幕看了大概三分鐘，心想：這哪裡簡單了。

但這就是軟體產品的真相——使用者看到的只是冰山一角，底下有一整座冰山在運作。

## 架構全貌

我用一張圖來呈現一個典型的軟體產品長什麼樣：
...
當我第一次把這張表畫出來，我才真正搞懂自己到底在做什麼。在那之前，我腦子裡就是一團「要做一個網站」的模糊概念。畫出來之後，每一層變成了一個可以單獨討論、單獨交給 AI 去處理的任務。
...
純 HTML/CSS/JS        React / Vue          Next.js / Nuxt
  │                     │                      │
  │                     │                      │
  ├─ 適合靜態網站        ├─ 適合互動多的應用       ├─ 適合需要 SEO 的產品
  ├─ 不需要框架          ├─ 元件化開發            ├─ 前後端整合
  └─ 改了馬上看到        └─ 學習曲線較高          └─ 功能最完整
```
...
我做皂花拾光（soap-flower-studio）的時候，就是用最簡單的純 HTML。一個 `index.html`、一個 `style.css`、一個 `main.js`，三個檔案搞定一整個品牌官網，有 Lightbox 相簿、有 FAQ、有 SEO、有 RWD（手機自動適應），部署到 Cloudflare 上，每月流量費用是零。

不是所有東西都需要用最新最潮的框架。有時候最樸素的方案反而最適合。

## 你需要能問出的問題

你不需要會寫 CSS，不需要知道 `useState` 是什麼，不需要搞懂 Webpack 設定。但你需要能問出這些問題：

1. **手機版看起來怎麼樣？** 現在超過一半的流量來自手機，你的網站在小螢幕上不能爛掉。
2. **搜尋引擎找得到嗎？** 這叫 SEO（Search Engine Optimization）。如果 Google 搜不到你的網站，等於不存在。
3. **載入速度夠快嗎？** 使用者等超過三秒就會離開。圖片太大、程式碼太肥，都會拖慢速度。
4. **視障者能用嗎？** 這叫可及性（Accessibility）。按鈕有沒有文字描述、對比度夠不夠高，這些不只是道德問題，在某些國家是法律要求。

這四個問題就夠了。問出來之後，AI 會幫你處理技術細節。
...
我做 TutorKit（智慧考卷解析）的時候，有一個功能是用 AI 解題——使用者上傳一張考卷照片，系統用 OCR 辨識題目，再用 Gemini AI 生成解答。

最初我想過一個偷懶的做法：直接在前端呼叫 Gemini API。少寫一層後端，省事。

然後我想到一件事：如果我把 Gemini 的 API Key 放在前端的程式碼裡，任何人只要按 F12 打開瀏覽器的開發者工具，就能看到那把 Key。然後他就可以拿我的 Key 去呼叫 Gemini，帳單算我的。
...
跟前端一樣，你不需要會寫後端程式。但你需要能問這些：

1. **這個資料放前端還後端處理？** 判斷標準就是上面說的——會有安全問題嗎？
2. **同時很多人用會怎樣？** 這叫併發（Concurrency）。十個人同時上傳考卷，後端撐得住嗎？
3. **如果這個步驟失敗了怎麼辦？** 付款到一半網路斷了，資料會不會不一致？
4. **敏感資料怎麼保護？** 使用者的個資、密碼、付款資訊，怎麼確保不會外洩？

問出這些問題，你就比大部分只會說「幫我做一個後端」的人強了。
...
## 當 AI 幫你建 API 時，你可以問什麼

1. **需要驗證身份嗎？** 任何人都能叫出賓客名單嗎？還是要登入才行？
2. **回傳什麼資料？** 我只需要考題標題，還是連答案一起回傳？
3. **出錯的時候回什麼？** 找不到這筆資料，要回傳什麼訊息？
4. **有速度限制嗎？** 如果有人一秒鐘連送一百個請求，怎麼辦？

第四點叫 Rate Limiting，謝卡醬就有設定——驗證賓客的 API 每分鐘每個 IP 只能打 5 次，防止有人暴力猜測賓客資料。這種細節 AI 會幫你做，但你要知道可以問。
...
```
┌─────── 資料庫 Table: guests ─────────────────┐
│  id   name      phone         table_no  rsvp  │
├─────────────────────────────────────────────-─┤
│  1    林雅婷    0912345678      3       true   │
│  2    陳建宏    0923456789      5       false  │
│  3    吳佳穎    0934567890      3       true   │
└──────────────────────────────────────────────┘
```
...
資料庫就是一個被軟體工程師強化過的 Excel——它多了併發控制、權限管理、資料驗證、自動備份這些能力。

## 你需要能問的問題

1. **資料存在哪個國家？** 如果你的使用者在台灣，資料存在美國的機房，每次讀取都要跨太平洋來回，會比較慢。而且某些產業有法規要求資料不能出境。
2. **備份機制是什麼？** 如果資料庫壞了，能回到什麼時候的狀態？
3. **誰能看到什麼資料？** 客服人員能看到使用者的密碼嗎？一般使用者能看到其他人的資料嗎？
4. **資料量大了之後會變慢嗎？** 一開始幾百筆沒感覺，到了十萬筆、百萬筆呢？
...
這不是什麼高深的架構決策，就是窮人的智慧。但它說明了一件事：資料庫的選擇不只是技術問題，也是成本問題。你要能跟 AI 說：「我這個專案的預算是零，有沒有免費的方案？」

---

# 第 8 章｜第三方服務：不用自己造輪子

現代軟體有個祕密：大部分功能不是自己寫的。

這不是偷懶，是聰明。你要做一個會員登入功能，與其自己寫一套密碼加密、Session 管理、忘記密碼流程，不如用 Google 或 Facebook 登入——人家的資安團隊比你多幾千人，他們處理這些問題的時間比你的產品存在的時間還長。
...
│  需求      │  常見服務             │  我用過的           │
├───────────┼──────────────────────┼────────────────────┤
│  使用者登入 │  Google OAuth,        │  Firebase Auth,    │
│           │  Auth0, Supabase Auth │  Supabase Auth     │
├───────────┼──────────────────────┼────────────────────┤
│  收款      │  Stripe, PayPal      │  PayUNI            │
│           │  綠界, PayUNI         │                    │
├───────────┼──────────────────────┼────────────────────┤
│  寄Email   │  SendGrid, Resend,   │  Resend            │
│           │  AWS SES             │                    │
├───────────┼──────────────────────┼────────────────────┤
│  AI 功能   │  OpenAI, Gemini,     │  Gemini, Claude,   │
│           │  Claude, Mistral     │  Mistral OCR       │
├───────────┼──────────────────────┼────────────────────┤
│  檔案儲存  │  AWS S3, Cloudflare  │  Cloudflare R2,    │
│           │  R2, Supabase Storage│  Supabase Storage  │
├───────────┼──────────────────────┼────────────────────┤
│  資料庫    │  Supabase, Firebase, │  Supabase          │
│           │  PlanetScale         │                    │
├───────────┼──────────────────────┼────────────────────┤
│  語音合成  │  Google TTS,         │  Edge-TTS          │
│           │  Edge-TTS, ElevenLabs│                    │
├───────────┼──────────────────────┼────────────────────┤
│  地圖      │  Google Maps,        │  Leaflet           │
│           │  Mapbox, Leaflet     │  (開源免費)          │
└───────────┴──────────────────────┴────────────────────┘
```
...
**倒閉**。你依賴的服務如果收掉了，你的功能就跟著消失。這幾年 Heroku 砍免費方案、Firebase 調整定價，都讓很多小專案措手不及。

**免費方案限制**。很多服務的免費方案有功能限制、速度限制、或是會在你的產品上顯示他們的 Logo。

應對方式不複雜：做選擇的時候多看一下替代方案，不要把所有雞蛋放在同一個籃子裡。如果某個服務真的掛了，你至少要知道可以換成什麼。
...
這大概是軟體開發裡最經典的一句話。你在自己的筆電上做好了一個網站，打開瀏覽器看起來完美無缺。然後你把網址傳給朋友，朋友說打不開。
...
1. **你會關機。** 你睡覺的時候筆電蓋起來，你的網站就下線了。全年無休不關機，你的電費帳單會很精彩。
...
這個流程叫 CI/CD（Continuous Integration / Continuous Deployment），白話講就是「推了新程式碼就自動部署」。你不需要每次都手動把檔案上傳到伺服器，推一下 Git 就搞定。
...
│  平台               │  適合什麼         │  我的專案              │
├────────────────────┼─────────────────┼──────────────────────┤
│  Cloudflare Pages  │  靜態網站、前端    │  Travel_blog, ToolBox,│
│                    │                 │  ExamBank 前端,       │
│                    │                 │  ohara-lab            │
├────────────────────┼─────────────────┼──────────────────────┤
│  Cloudflare Workers│  輕量後端、邊緣運算 │  weddingcard-saas,   │
│                    │                 │  soap-flower-studio   │
├────────────────────┼─────────────────┼──────────────────────┤
│  Google Cloud Run  │  完整後端、容器化  │  TutorKit, ExamBank   │
│                    │  需要較多運算資源  │  後端, GuideCard,     │
│                    │                 │  MemoryTale, CardButler│
├────────────────────┼─────────────────┼──────────────────────┤
...
1. **出事了能回到上一版嗎？** 這叫回滾（Rollback）。新版本有 bug，能不能一鍵退回上一個正常的版本？
2. **環境變數怎麼管理？** API Key、資料庫密碼這些不能寫在程式碼裡，要放在部署平台的環境變數設定中。
3. **費用怎麼算？** 有些平台按流量收費，有些按運算時間收費。你的產品如果突然爆紅，帳單會不會跟著爆炸？
4. **流量大了能自動擴展嗎？** 平常一天一百人用，某天上了新聞突然一萬人湧進來，撐得住嗎？

我曾經在 Google Cloud Run 上被帳單嚇到過——幾個後端服務加上 Container Registry 的儲存費，每個月零零總總加起來比我預期的高。免費方案的額度用完之後，那些小數字會慢慢累積。所以費用這題，一定要問。
...
這是一個很重要但很反直覺的觀念：你的系統一定會壞。

不是「可能會壞」，是「一定會壞」。只是時間和方式的問題。資料庫連不上、第三方 API 掛了、某段程式碼有一個只在特定條件下才會觸發的 bug、SSL 憑證過期了沒人注意到。

重點不是「怎麼讓系統永遠不壞」——那是不可能的。重點是**壞了多快能發現、多快能修好**。
...
有一次我某個部署在 Cloud Run 上的服務，半夜突然開始回傳 502 錯誤。原因是 Container 啟動失敗——前一天更新了一個套件的版本，新版本跟另一個套件不相容，但本機測試的時候沒抓到，因為本機的環境跟 Cloud Run 的環境有微妙的差異。
...
不是我沒有監控的知識——我知道該設。就是懶。懶的代價就是五個小時的服務中斷。

## 維運的心態

做產品跟寫程式是不一樣的心態。寫程式的時候你在「創造」，維運的時候你在「守護」。守護沒有創造那麼刺激，但如果你不守護，你創造的東西會慢慢腐爛。

套件不更新，安全漏洞就會累積。錯誤不處理，使用者就會流失。費用不監控，某天帳單會教你做人。
...
很多人以為用 AI 寫程式最難的部分是「不會寫程式」。

不是。最難的是你不知道自己要什麼。

這聽起來很玄，但你想想：如果你去餐廳點菜，「我要一碗牛肉麵」跟「我要吃東西」，廚師收到的指令品質差多少？AI 也一樣。它不怕你不懂技術，它怕你給的指令模糊到它只能猜。而 AI 猜出來的東西，有時候看起來很像那麼回事，但其實完全不是你要的。

我在做 MemoryTale（一個 AI 口述歷史平台）的時候，經歷了一段很痛的學習過程。一開始我跟 AI 說：「幫我做一個可以記錄老人家故事的網站。」它做出來了，但跟我想的完全不一樣——它做了一個部落格。技術上沒錯，但那不是我要的東西。

後來我慢慢摸索出一套方法，我叫它「四層對話法」。不是什麼高深理論，就是我自己踩坑踩出來的。
...
AI 規劃完架構後，會丟一堆技術選項給你。這時候你不需要懂每個選項的細節，但你需要問對的問題。

比如 AI 跟我說：「語音轉文字可以用 Google Speech-to-Text、Whisper、或 Deepgram。」我不懂這三個有什麼差別，所以我問：「這三個方案，哪個最便宜？哪個中文辨識最準？哪個最容易整合？幫我列一個比較表。」

AI 列完比較表，我就能用「我的情境」來做決定。不是選技術上最強的，而是選最適合我的。這個概念後面幾章會反覆出現。

### 第三層：驗收

東西做出來了，你要檢查。怎麼檢查？不是去看程式碼（你又看不懂），而是去用。打開網頁，點每個按鈕，走一遍流程。

「錄音功能正常嗎？」「轉出來的文字對嗎？」「故事文章的格式好讀嗎？」

這些都是你可以判斷的。你不需要懂程式，你只需要站在使用者的角度看結果。

### 第四層：迭代

驗收完發現問題，回頭跟 AI 說：「錄音超過十分鐘會斷掉，這要修。另外，故事文章的開頭太平淡了，能不能讓 AI 自動生成一個比較有吸引力的第一段？」

然後又回到第一層：新的目標、新的決策、新的驗收。循環往復。
...
中間那個層次最難拿捏，但也最重要。說太少 AI 會亂猜，說太多你反而限制了它。我的經驗是：**把自己當成一個很會描述需求的使用者，而不是一個在寫規格書的工程師**。

你可能會問：「那我怎麼知道自己的指令是不是在對的層次？」有個很簡單的判斷方法——如果你的指令裡出現了技術名詞，而且你其實不太確定那些名詞的意思，那你大概說太具體了。退一步，用白話文重新講一次你要什麼。
...
聽起來很偷懶？也許吧。但我覺得這才是正確的分工——AI 負責「研究」，你負責「判斷」。研究是可以被自動化的，判斷不行。因為判斷需要你對自己的情境有理解，而這個只有你自己知道。
...
做技術決策最大的陷阱是追求完美。永遠有更新、更強、更便宜的方案，但你的專案不會等你研究完。**選一個「夠好」的方案，快速驗證，不行再換——這比花三週挑出「最佳方案」然後才開始動手，效率高太多了。**
...
AI 很積極、很主動。你叫它做 A，它常常會「順手」把 B、C、D 也做了。有時候這很方便，但有時候會搞出大災難。
...
這就是為什麼禁止事項這麼重要。你不可能事先想到 AI 會做的每一件好事，但你可以根據經驗，把它做過的壞事一條一條記下來。每次被坑，就多加一條規則。久了，你的 CLAUDE.md 就變成一份「血淚教訓清單」，而 AI 會乖乖遵守。
...
謝卡醬的第二輪：我加了十個配色方案（本來只有三個灰不拉嘰的底色）、調整了文字的字體跟排版、讓卡片預覽看起來至少像是這個年代的產品。
...
迭代思維的核心精神是：你不需要一開始就做對所有事情。你只需要每一輪比上一輪好一點。聽起來很慢？其實比「想清楚再動手」快多了。因為「想清楚」是一個不存在的終點——你永遠覺得還沒想完。
...
這聽起來好像很理所當然？但你想想，如果你不會寫程式，你怎麼判斷 AI 寫的程式碼有沒有問題？很多人的做法是——不判斷。AI 說做完了，看起來能動，就上線了。

這是很危險的。

## 一次慘痛的教訓

有一次，AI 幫我做了一個功能，需要呼叫外部 API。做完之後我試了一下，功能正常。但我後來無意間多看了一眼程式碼（雖然看不太懂，但有些東西還是認得出來的），發現 AI 直接把 API Key 寫在前端的 JavaScript 裡面。
...
你不需要懂什麼是環境變數、什麼是身份驗證。你只需要知道：AI 自己說有風險的地方，就是要修的地方。
...
我知道很多人會覺得：「我又看不懂程式碼，怎麼驗收？」

驗收不只是看程式碼。我的驗收流程通常是這樣的：
...
驗收的能力是可以培養的。你做的專案越多，你就越知道該檢查什麼。第一個專案可能只會檢查「功能有沒有動」，到了第十個專案，你會本能地去檢查安全性、效能、錯誤處理。

**不要因為看不懂程式碼就放棄驗收。** 你不需要看懂程式碼，你需要的是會問問題。而問問題這件事，人類比 AI 強多了。
